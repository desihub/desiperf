...
class FiberPosPage(Page):
    ...
    def time_series_plot(self):
        print(self.page.tools)
        self.ts1 = figure(plot_width=900, plot_height=200, tools=self.page.tools, x_axis_type='datetime', active_drag="xbox_select")
        if self.data_source is not None:
            self.ts1.line(self.x_select.value, self.y_select.value, source=self.data_source)
            self.ts1.circle(self.x_select.value, self.y_select.value, size=1, source=self.data_source, color=None, selection_color="orange")
